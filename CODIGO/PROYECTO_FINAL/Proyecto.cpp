...
void borrarPaciente (listaProvincias cabeza,int cedulaa)
{
	listaProvincias aux;
	listaPacientes aux1,anterior;
	char* fechaActual= get_time();
	
	if (cabeza==NULL)
	file<<fechaActual<<"Lista vacia: "<<&aux <<endl;
	else
	{
		aux=cabeza;
		vector <int> provincia= guessProvincia(cedulaa);
		do
		{
			if(aux->provincia==provincia[0])
			{
				if(aux->paciente==NULL)
				cout <<"no doctor"<< endl;
				else
				...
				aux1=aux->paciente;
				anterior=aux1;
					if(aux->paciente->cedula==cedulaa)
					{
						aux->paciente=aux1->sig;
						aux1->sig=NULL;
						free(aux1);
					...
					else
					{
						while(aux1!=NULL)
						{
							if(aux1->cedula==cedulaa && aux1->sig!=NULL)
							{
...
void borrarDoctor (lista cabeza,int identificador)
{
	lista aux;
	listaDocs aux1,anterior;
	char* fechaActual= get_time();
	...
	else
	{
		...
		do
		{
			if(aux->especialidad==especialidad)
			{
				...
				aux1=aux->doc;
				anterior=aux1;
					if(aux->doc->codigo==identificador)
					{
						aux->doc=aux1->sig;
						aux1->sig=NULL;
						free(aux1);
					...
					else
					{
						while(aux1!=NULL)
						{
							if(aux1->codigo==identificador && aux1->sig!=NULL)
							{
								anterior->sig=aux1->sig;
								aux1->sig=NULL;
								free(aux1);
...
void borrarDoctor(lista &cab, int identificador){
    ...
    while(aux!=NULL){
        if(aux->especialidad==especialidad){
        	//cout<<aux->especialidad;
           //oaux2=aux->doc;
            //cout<<aux2->codigo;
            //cout<<identificador;
            //system("pause");
        	/*if((aux2->codigo==identificador) && (aux2->sig!=NULL)){
			cout<<aux2->codigo;
            cout<<identificador;
            system("pause");
        		    aux->doc=aux2->sig;
